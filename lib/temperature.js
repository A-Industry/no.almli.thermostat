...
let resolveOnoff = async function (aDevice, temperature, targetTemp, settings, opts, contactAlarm) {
    ...
    if (contactAlarm || !mainOnoff) {
        ...
        let pctOfHeating = aDevice._ctr.update(temperature);
        ...
        if(pctOfHeating < 0) {
            pctOfHeating = 0;
        } else if(pctOfHeating > 100) {
            pctOfHeating = 100;
        }

        aDevice.generateHeatingPlan(pctOfHeating);

        await aDevice.setCapabilityValue('pid_value', pctOfHeating ? Math.round(pctOfHeating * 10) / 10 : 0)
            .catch(e =>
                console.error("Error writing PID value", pctOfHeating ? Math.round(pctOfHeating * 10) / 10 : 0, e)
            );

        const heatingAllowed = aDevice.heatingAllowed();
