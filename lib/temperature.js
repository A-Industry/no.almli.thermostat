...
let resolveOnoff = async function (aDevice, temperature, targetTemp, settings, opts, contactAlarm) {
    ...
    if (contactAlarm || !mainOnoff) {
        ...
        aDevice._ctr.setTarget(targetTemp);
        let minsOfHeatingPer2Hours = Math.round(aDevice._ctr.update(temperature));
        const heatingAllowed = aDevice.heatingAllowed(minsOfHeatingPer2Hours);

        if(minsOfHeatingPer2Hours > 61) {
            minsOfHeatingPer2Hours = 61;
        } else if(minsOfHeatingPer2Hours < 0) {
            minsOfHeatingPer2Hours = 0;
        }

        aDevice.log('invert: ' + invert);
        aDevice.log('heatingAllowed: ' + heatingAllowed);
        aDevice.log('minsOfHeatingPer2Hours: ' + minsOfHeatingPer2Hours);

        onoff = heatingAllowed === !invert;


        await aDevice.setCapabilityValue('pid_value', minsOfHeatingPer2Hours).catch(console.error);
    }
